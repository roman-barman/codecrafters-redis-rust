    ...
    pub fn handle_request(&self, request: &str) -> Result<RespType, anyhow::Error> {
        ...
        match command {
            Ok(c) => match c {
                Commands::Ping => self
                    .mediator
                    .send(Box::new(PingCommand))
                    .map(|x| RespType::SimpleString(x)),
                Commands::Echo(s) => self
                    .mediator
                    .send(Box::new(EchoCommand::new(s.to_string())))
                    .map(|x| RespType::BulkString(x)),
                Commands::Get(s) => self
                    .mediator
                    .send(Box::new(GetCommand::new(s.to_string())))
                    .map(|x| x.map_or(RespType::NullBulkString, |s| RespType::BulkString(s))),
                Commands::Set(arg) => self
                    .mediator
                    .send(Box::new(SetCommand::new(
                        arg.key.to_string(),
                        arg.value.to_string(),
                        arg.expiry,
                    )))
                    .map(|x| RespType::BulkString(x)),
                Commands::GetConfig(arg) => self
                    .mediator
                    .send(Box::new(GetConfigCommand::new(arg.to_string())))
                    .map(|x| {
                        RespType::Array(
                            x.split(' ')
